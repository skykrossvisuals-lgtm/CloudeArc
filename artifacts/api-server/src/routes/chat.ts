...
// ── Model chain ────────────────────────────────────────────────────────────────
// Uses environment variables MODEL_AGENT and MODEL_FALLBACK.

const PRIMARY_MODEL = process.env.MODEL_AGENT ?? "anthropic/claude-3.5-sonnet";
const FALLBACK_MODEL = process.env.MODEL_FALLBACK ?? "anthropic/claude-3.5-sonnet";
...
async function callAgentStreaming(
  ...
  for (const model of chain) {
    ...
    if (!response.ok) {
      ...
      const providerName = API_PROVIDER === "nvidia" ? "NVIDIA" : "OpenRouter";
      const err = new Error(`${providerName} API ${response.status} (${model}): ${body.slice(0, 300)}`);
